...
from .config_lekiwi_base import LeKiwiBaseClientConfig
...
class LeKiwiClient(Robot):
    ...
    def connect(self) -> None:
        ...
        if self._is_connected:
            raise DeviceAlreadyConnectedError(
                "LeKiwi Base Daemon is already connected. Do not run `robot.connect()` twice."
            )
        ...
        if self.zmq_observation_socket not in socks or socks[self.zmq_observation_socket] != zmq.POLLIN:
            raise DeviceNotConnectedError("Timeout waiting for LeKiwi Base Host to connect expired.")
    ...
    def _remote_state_from_obs(
        ...
        return None, {"observation.state": state_vec}
    ...
    def _get_data(self) -> Tuple[Dict[str, np.ndarray], Dict[str, Any], Dict[str, Any]]:
        ...
        try:
            _, new_state = self._remote_state_from_obs(observation)
    ...
    def get_observation(self) -> dict[str, Any]:
        ...
        if not self._is_connected:
            raise DeviceNotConnectedError("LeKiwiBaseClient is not connected. You need to run `robot.connect()`.")

        _, obs_dict = self._get_data()
    ...
    def __from_xboxController_to_base_action(self, controller_state):
        ...
        if controller_state['left_stick_y'] <= -0.9:
            x_cmd += xy_speed
        # left stick down: backward    
        if controller_state['left_stick_y'] >= 0.9:
            x_cmd -= xy_speed
        # left stick left: left    
        if controller_state['left_stick_x'] <= -0.9:
            y_cmd += xy_speed
        # left stick left: left
        if controller_state['left_stick_x'] >= 0.9:
            y_cmd -= xy_speed
        # right stick left: rotate left
        if controller_state['right_stick_x'] <= -0.9:
            theta_cmd += theta_speed
    ...
    def disconnect(self):
        ...
        if not self._is_connected:
            raise DeviceNotConnectedError(
                "LeKiwi Base is not connected. You need to run `robot.connect()` before disconnecting."
            )
