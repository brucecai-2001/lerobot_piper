...
from .config_so101_remote import SO101ClientConfig

class SO101Client(Robot):
    """
        SO101 Client, Send action from so101 leader to the remote so101 follower(host)
    """
    ...
    def connect(self) -> None:
        ...
        if self._is_connected:
            raise DeviceAlreadyConnectedError(
                "SO101 Daemon is already connected. Do not run `robot.connect()` twice."
            )
