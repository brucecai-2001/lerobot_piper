...
from .config_so101_bimanual_remote import SO101BimanualClientConfig

class SO101BimanualClient(Robot):
    config_class = SO101BimanualClientConfig
    name = "so101_bimanual_client"

    def __init__(self, config: SO101BimanualClientConfig):
    ...
    def connect(self) -> None:
        ...
        if self._is_connected:
            raise DeviceAlreadyConnectedError(
                "SO101 Bimanual Daemon is already connected. Do not run `robot.connect()` twice."
            )
    ...
    def disconnect(self):
        ...
        if not self._is_connected:
            raise DeviceNotConnectedError(
                "SO101 is not connected. You need to run `robot.connect()` before disconnecting."
            )
    ...
    def get_observation(self) -> dict[str, Any]:
        """
        Capture observations from the remote robot: 
        current follower arm positions,
        and a camera frame. Receives over ZMQ
        """
        if not self._is_connected:
            raise DeviceNotConnectedError("SO101Client is not connected. You need to run `robot.connect()`.")
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        """Command SO101 to move to a target joint configuration. Translates to motor space + sends over ZMQ
        ...
        if not self._is_connected:
            raise DeviceNotConnectedError(
                "SO101 is not connected. You need to run `robot.connect()`."
            )
